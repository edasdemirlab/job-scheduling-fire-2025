...
n_vehicles <- problem_parameters %>% filter(parameter == "number_of_vehicles") %>% pull(value)
...
coordinates[base_node_id,"node_state"] <- 6
...
coordinates <- coordinates %>% mutate(node_value_at_start  = replace(node_value_at_start, node_id %in% base_node_id, 0))
coordinates <- coordinates %>% mutate(node_degradation_rate  = replace(node_degradation_rate, node_id %in% base_node_id, 0))
