...
generate_dist_matrix <- function(param){
  ...
  dist_matrix <- dist_matrix/param$moving_speed # in terms of travel time
  return(dist_matrix)
}
...
#   region_map_df["grid_value_at_start"] <- param$grid_value_at_start
#   region_map_df["grid_degradation_rate"] <- param$grid_degradation_rate
#   region_map_df["grid_amelioration_rate"] <- param$grid_amelioration_rate
#   region_map_df[param$water_grid_id,"grid_amelioration_rate"] = NA
#   region_map_df[param$water_grid_id,"grid_degradation_rate"] = NA
#   region_map_df[param$water_grid_id,"grid_value_at_start"] = NA
...
generate_fire_df <- function(param, region_map_df) {
  # check active fires
  fire_size_start <- 0
  fire_size_max <- param$grid_area
  fire_map_df <- region_map_df
  fire_map_df["fire_size_now"] <- 0
  fire_map_df["fire_time_start"] <- 0
  fire_map_df <- fire_map_df %>% mutate(fire_time_max = (fire_size_max - fire_size_start) / grid_degradation_rate )
  fire_map_df <- fire_map_df %>% mutate(fire_time_end = fire_time_max + (fire_size_max / grid_amelioration_rate))
  fire_map_df <- fire_map_df %>% mutate(value_now = grid_value_at_start)
  fire_map_df <- fire_map_df %>% mutate(value_degradation_rate = grid_value_at_start / fire_time_end)
  return(fire_map_df)
}
