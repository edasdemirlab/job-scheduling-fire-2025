...
def clustering_model_solve(mip_inputs):
    print("\n" + "-" * 80)
    print("📌 Solving clustering model...")
    print("-" * 80 + "\n")
    ...
    print("\n" + "-" * 80)
    print("📌 Clustering model is solved.")
    print("-" * 80 + "\n")
    return mip_inputs

def exact_model_solve(mip_inputs):
    ...
    if mip_inputs.algorithm in ['em', 'cem'] or (mip_inputs.algorithm in ['e-rlm', "ce-rlm"] and mip_inputs.hybrid_mode == 'em'):
    ...
    if mip_inputs.algorithm in ['em', 'cem']:
    ...
    elif mip_inputs.algorithm in ['e-rlm', 'ce-rlm'] and mip_inputs.hybrid_mode == 'em':
        print("\n" + "-" * 80)
        print("📌 Solving exact model...")
        print("-" * 80 + "\n")
        ...
        print("\n" + "-" * 80)
        print("📌 Solving refill lazy-constrained model...")
        print("-" * 80 + "\n")
        if mip_inputs.algorithm in ['e-rlm', 'ce-rlm'] and mip_inputs.hybrid_mode == 'rlm':
            print("\n" + "-" * 80)
    ...
    if model.Status == GRB.Status.INFEASIBLE:
        ...
        if mip_inputs.algorithm in ['e-rlm', 'ce-rlm'] and mip_inputs.hybrid_mode == 'em':
            ...
            if mip_inputs.algorithm in ["cem", "crlm"]:
            ...
            if mip_inputs.algorithm in ["ce-rlm"]:
                ...
                global_results_df["run_time_original_mip"] = mip_inputs.run_time_original_mip
                global_results_df["run_time_callback_mip"] = run_time_cpu
                global_results_df["run_time_total"] = mip_inputs.run_time_cpu_clustering + mip_inputs.run_time_original_mip + run_time_cpu
            ...
            if mip_inputs.experiment_mode in ["single_run"]:
                writer_file_name = os.path.join('outputs', "{0}_{1}_results_{2}_nodes_{3}.xlsx".format(mip_inputs.algorithm, mip_inputs.experiment_mode,
                                                                                                  mip_inputs.n_nodes,
                ...
                if mip_inputs.experiment_mode in ["cem", "crlm", "ce-rlm"]:
